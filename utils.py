...
def get_data(url):
    """
    Функция получает json cписок с интернета
    :return: None, Статус ответа
    """
    try:
        response = requests.get(url)
        if response.status_code == 200:
            return response.json(), "INFO: Данные получены успешно!"
        return None, f"WARNING: Cтатус ответа {response.status_code}"
...
def get_filtered_data(data, filtered_empty_from=False):
    """
    Функция убирает из списка, словари без "from"
    :return: data
    """
...
def get_last_data(data, count_last_values):
    """
    Функция возвращает отсортированный список по дате, количество транзакций определяется на входе функции
    :return: data
    """
...
def get_formatted_data(data):
    """
    Функция возвращает отформатированный текст из data
